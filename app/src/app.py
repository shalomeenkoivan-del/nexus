...
def add_room():
    ...
    secure = data.get('secure', 'public')
    password = data.get('password')

    if room_storage.room_exists(room_name):
        room_meta = room_storage.get_room_meta(room_name)
        if room_meta.get("secure") == "private":
            if user_id != room_meta["creator"]:
                if password is None:
                    return jsonify({
                        'success': False,
                        'message': 'This room is private. Please provide a password.',
                        'needs_password': True
                    })
                if password != room_meta.get("password"):
                    return jsonify({
                        'success': False,
                        'message': 'Invalid password, acces denied!',
                        'needs_password': True
                    })
    ...
    if room_storage.room_exists(room_name):
        with chat_rooms_lock:
            chat_rooms[room_name].append(join_msg)
    else:    
        room_storage.create_room(room_name, username, secure, password)
        with chat_rooms_lock:
            if room_name in chat_rooms:
                pass
            else:
                chat_rooms[room_name] = [{
                    "user": "SYSTEM", "user_id": "SYSTEM",
                    "message": f"Room '{room_name}' created by {username}! To manage your room the creator ({username}) can right click on it (supposed options are: locking it to make private with password or fully deleting with data).",
                    "time": timestamp, "system": True
                }]
...
@app.route('/lock_room', methods=['POST'])
def lock_room():
    if not session.get('logged_in'):
        return jsonify({'success': False, 'message': 'Login first!'})
    ...
    if not room_storage.room_exists(room_name):
        return jsonify({'success': False, 'message': 'Room not found!'})
    
    room_meta = room_storage.get_room_meta(room_name)


    if username != room_meta.get('creator'):
        return jsonify({'success': False, 'message': 'Only creator can lock room!'})
    
    room_meta['secure'] = 'private'
    room_meta['password'] = new_password if new_password else None

    room_storage.rooms_file_storage.save(f"{room_name}.json", room_meta)

    timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
    with chat_rooms_lock:
        chat_rooms[room_name].append({
            "user": "SYSTEM", "user_id": "SYSTEM",
            "message": f"Room locked by {username}! Now private.",
            "time": timestamp, "system": True
        })
    
    return jsonify({
        'success': True, 
        'message': f'Room "{room_name}" locked! Password: {new_password or "none"}'
    })
...
def unlock_room():
    ...
    if username != room_meta.get('creator'):
        return jsonify({'success': False, 'message': 'Only creator can unlock room!'})
    
    room_meta['secure'] = 'public'
    room_meta['password'] = None
    ...
    with chat_rooms_lock:
        chat_rooms[room_name].append({
            "user": "SYSTEM", "user_id": "SYSTEM",
            "message": f"Room unlocked by {username}! Now public.",
            "time": timestamp, "system": True
        })
    
    return jsonify({
        'success': True, 
        'message': f'Room "{room_name}" unlocked! No password!'
    })

@app.route('/rmrf_room', methods=['POST'])
def rmrf_room():
    if not session.get('logged_in'):
        return jsonify({'success': False, 'message': 'Login first!'})
    ...
    if room_name == 'general':
        return jsonify({'success': False, 'message': 'Cannot DELETE general!'})
    ...
    if username != room_meta.get('creator'):
        return jsonify({'success': False, 'message': 'Only creator can DELETE room!'})

    room_file = room_storage.rooms_file_storage.base_path / f"{room_name}.json"
    if room_file.exists():
        room_file.unlink()

    user_rooms_dir = DIRS["user_rooms"]
    for user_file in user_rooms_dir.glob("*.json"):
        try:
    ...
    with chat_rooms_lock:
        chat_rooms_snapshot = room_storage.load_global_chat_rooms()
        if room_name in chat_rooms_snapshot:
            del chat_rooms_snapshot[room_name]
            room_storage.save_global_chat_rooms(chat_rooms_snapshot)
        
        chat_rooms.pop(room_name, None)

    timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
    with chat_rooms_lock:
        chat_rooms["general"].append({
            "user": "SYSTEM", "user_id": "SYSTEM",
            "message": f"Room '{room_name}' PERMANENTLY DELETED by {username}!",
            "time": timestamp, "system": True
        })

    return jsonify({
        'success': True, 
        'message': f'Room "{room_name}" COMPLETELY DESTROYED!'
    })

@app.route('/room_status')
def room_status():
    room_name = request.args.get('room', '').lower()
    
    global_rooms = room_storage.load_global_room_names()
    if room_name in global_rooms:
        return jsonify({'privacy': 'global'})

    if not room_storage.room_exists(room_name):
        return jsonify({'privacy': 'none'})
    
    room_meta = room_storage.get_room_meta(room_name)
    
    if room_meta.get('secure') == 'private':
        return jsonify({'privacy': 'private', 'icon': '/data/images/room-private.png'})
    else:
        return jsonify({'privacy': 'public', 'icon': '/data/images/room-public.png'})

@app.route('/room_status_crown')
def room_status_crown():
    room_name = request.args.get('room', '').lower()
    ...
    room_meta = room_storage.get_room_meta(room_name)
    is_creator = username == room_meta.get('creator', '')
    
    return jsonify({
        'is_creator': is_creator,
        'icon': '/data/images/room-creator.png' if is_creator else None
    })
...
if __name__ == '__main__':
    for path in [DIRS["users"], DIRS["images"], DIRS["user_rooms"], DIRS["rooms_management"]]:
